...
Go back to the Fluffy Deploy stage in the GUI and see the edited step.  This change is not saved!  Click Save to commit the changes to the master repo.  See appendix A for complete solution.
...
Save and run the Pipeline.  After a successful test migrate the branch to master and run.  This time the when directive will be true and the Pipeline will pause for input.  See Appendix E for the solution.

## Documentation

The Declarative Pipeline Syntax: Up to date reference for building Declarative Pipelines
